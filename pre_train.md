...
原始语料的形式官方给了一个sample_text.txt的例子.
```text
...
每句话为一行,每份上下文用空行间隔开.
因为训练时的`segment_id` 对应于文档中的第几个句子,因此不同的句子需要做一个区分,而在训练上文预测下文的任务时,
显然不同的文档之间是不能作为上下文的,因此不同的文档也要作区分.

通过运行`create_pretraining_data.py`就能将原始文本转化为预训练数据.
为了更加形象的理解这一步的操作,我们将该文件中的几个主要操作抽出来,在示例文本上看一下效果,具体就看这个文件,写了大量注释
[crate_pretraining_interpretation.py](./crate_pretraining_interpretation.py)

得到了预训练数据,就可以进行pre train操作了.pre train只要运行`run_pretraining.py`就可以了.
